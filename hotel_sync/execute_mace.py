...
def open_agent_file(file):
    if file.split('.')[2] == 'inp':
        ...
        gms_out_filename = subprocess.run([fegms_sh, file], stdout=subprocess.PIPE)
        # 同时生成 dat文件，用于下一个。
...
def get_q(file_name):
    ...
    returncode = subprocess.run([extract_gradmatrix_sh, file_name_out, file_name_q])
...
def make_mace(file_first, gamma, tolerance):
    ...
    while not done:
        ...
        subprocess.run([extract_dat_to_inp_sh])
