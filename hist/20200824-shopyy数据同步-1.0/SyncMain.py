...
from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QDesktopWidget, QPushButton, QLabel, QLineEdit, QDialog,QCheckBox
...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self._title = "ShopYY产品同步 " + config.get("api", "domain_name")
        ...
        # another line
        self.line_product_id = QLineEdit('ID', self)
        ...
        y = y + 44
        w = 100
        self.line_product_id.setGeometry(x, y + 10, w, h - 20)
        x = x + w + 10
        w = 60
    ...
    def sync_select(self):
        ...
        init_file = os.path.normpath(os.path.join(os.curdir, "config", "ymcart.ini"))
        config.read(init_file)
        select_goods_id = self.line_product_id.text()
        if len(select_goods_id) == 0:
            if config.has_option("api", "goods_id"):
                select_goods_id = config.get("api", "goods_id")
            else:
                select_goods_id = ""
        self.worker.select_goods_id = select_goods_id
        self.thread.start()
    ...
    def find_product(self):
        try:
            productID = self.line_product_id.text()
            dao = SyncDao.EtlDao()
            product_image = dao.select_product_image(int(productID))
            if not product_image:
                self.showDialog("商品:" + productID + " 没有信息")
            else:
    ...
    def print_output(self, item):
        if "alert" in item:
            ...
            dao = SyncDao.EtlDao()
            product_image = dao.select_product_image(ProductID)
            if not product_image:
                self.showDialog("商品:" + str(ProductID) + " 没有信息")
            else:
                self.line_product_id.setText(str(ProductID))
