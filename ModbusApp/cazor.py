...
try:
    ...
    try:  ## for command line showerror does not work
        ...
        print(
            'Import errror. docopt package was not found on your system. Please install it using the command: "pip install docopt"')
...
try:
    ...
    try:  ## for command line showerror does not work
        ...
        print(
            'Import errror. pymodbus package was not found on your system. Please install it using the command: "pip install pymodbus"')
...
class Gui:
    def __init__(self, master):
        ...
        Button(filesframe, text='…', command=(self.selectImportFile)).grid(row=0, column=2,
                                                                           sticky='W')  ## opens dialog to choose file from
        ...
        Button(filesframe, text='…', command=(self.selectLoggerDataFile)).grid(row=1, column=2,
                                                                               sticky='W')  ## opens dialog to choose file from
        ...
        Button(filesframe, text='⟲ Re-Read Configuration', command=(self.displaySettings)).grid(row=3, column=0,
                                                                                                sticky='W')  ## triggers re-read of the configuration file
        Button(filesframe, text='⤓ Save Current Configuration', command=(inout.writeExportFile)).grid(row=3, column=1,
                                                                                                      sticky='W')  ## triggers re-read of the configuration file
        ...
        self.commButton = Button(controlframe, text='▶ Start Communication', bg='lightblue',
                                 command=self.startCommunication)
        self.commButton.grid(row=0, column=1, sticky='W')

        ## fields for configuring the data connection
        #
        Label(self.settingsframe, text='Communication Connection Settings', font='-weight bold').grid(columnspan=4,
                                                                                                      sticky='W')
        ...
        Button(self.settingsframe, text='⮴ Update Settings', bg='lightgreen', command=(self.updateCommSettings)).grid(
        ...
        Button(self.datasettingsframe, text='+', font='-weight bold', bg='lightyellow',
               command=(self.addNewDataset)).grid(row=2, column=6)
    ...
    def displayCommSettings(self):
        ...
        self.input_ipaddress.bind('<Return>',
                                  self.updateCommSettings)  ## enable the Entry to update without button click
    ...
    def updateCommSettings(self, *args):
        ...
        if self.input_loginterval.get() != '':
            ...
            try:
                ...
                messagebox.showerror('Logger Interval Error',
                                     'the value you entered seems not to be a valid logger intervall')
                return
    ...
    def selectImportFile(self):
        data.inifilename = filedialog.askopenfilename(title='Choose Configuration File', defaultextension='.ini',
    ...
    def selectLoggerDataFile(self):
        data.logfilename = filedialog.asksaveasfilename(initialfile=data.logfilename,
                                                        title='Choose File for Logger Data', defaultextension='.csv',
